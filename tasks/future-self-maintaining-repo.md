...
throughout. See "Researched: a separate bot identity" below for
whether a `claude[bot]`-style identity is available to this loop.

### Researched: a separate bot identity

Jim asked to look into whether comments and commits can show as a
distinct bot rather than him. Findings, from Anthropic's own
documentation
([code.claude.com/docs/en/github-actions](https://code.claude.com/docs/en/github-actions))
and a general web search, not from testing:

- **A separate identity exists, but for a different product.** The
  Claude GitHub App (`github.com/apps/claude`) backs
  `anthropics/claude-code-action`, a GitHub Actions workflow
  integration. Its own documentation states plainly: "`github_token`:
  Token for GitHub operations. When omitted, the Claude Code GitHub
  Action authenticates as the Claude GitHub App." Left at its
  default, a run of that action comments and commits as the App's
  own account, not as whoever installed it. The documentation's own
  troubleshooting section confirms this by contrast, warning that
  passing GitHub's `GITHUB_TOKEN` instead makes commits show as
  `github-actions[bot]` and stop triggering CI, and that removing it
  restores the App's own identity.
- **That product is not the one this loop runs on.** The same page
  states outright that "Claude Code in the cloud" (the product this
  Routine's fired sessions run as) and `claude-code-action` (the
  GitHub Actions workflow) are separate products, sharing the Claude
  GitHub App installation only for two named features, Code Review
  and pull request auto-fix in cloud sessions, neither of which is
  what `MAINTAINER-RUN.md` does. A cloud session's own GitHub MCP
  tools, the ones this loop calls throughout, were not named among
  those features, and `get_me`'s own result (`jimbarritt`, not a bot)
  is direct evidence they authenticate through the personal
  connection instead.
- **No lighter-weight switch found.** No flag on `create_trigger`,
  `create_session`, or the GitHub MCP tools themselves, in what this
  search covered, redirects a cloud session's GitHub calls from the
  personal connection to the Claude GitHub App's identity. This is an
  absence in what was found, not a confirmed impossibility; it was
  not tested directly.
- **The only route found is the trigger this plan already passed
  over.** Getting bot attribution for real would mean switching this
  loop's trigger mechanism from a Routine firing a cloud session to a
  GitHub Actions workflow running `claude-code-action` on a schedule
  or on `issues.opened`, exactly Trigger option 2 under "Trigger: how
  an issue starts a session" above, already set aside for the first
  version because it loses Routines, `send_later`, Remote Control,
  and the planning-branch worktree convention this design leans on
  throughout. A hybrid, the cloud session doing the real work as now
  but a separate GitHub Actions step posting the final comment and
  closing the issue under the App's identity, would recover
  attribution without losing those, at the cost of two systems
  instead of one and a hand-off between them. Neither built nor
  recommended here; Jim's call once he returns to this.
